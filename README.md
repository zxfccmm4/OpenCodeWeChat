...
如果当前通过 `OPENCODE_AGENT=omo` 或 `OPENCODE_AGENT=sisyphus` 使用 OMO，可以在微信消息前加轻量指令前缀，桥接层会自动把它们翻译成更贴近官方 OMO 工作流的提示：

- `#ulw 你的需求` 或 `#ultrawork 你的需求`：映射到官方 `ultrawork` 全自动模式
- `#plan 你的需求`：映射到 Prometheus / `@plan` 的规划语义
- `#start 你的需求`：映射到 Atlas / `/start-work` 的执行语义；如果同一微信用户最近发过 `#plan`，桥接层会自动附带最近一次规划结果
- `#delegate 你的需求`：明确提示 OMO 优先采用委派、并行调查或多智能体拆分
- `#deep 你的需求`：要求 OMO 进入更深入的分析模式，必要时启用更强的推理或咨询能力
- `#review 你的需求`：切到评审视角，优先找风险、回归和缺失测试
- `#summary 你的需求`：把当前结论或进展压缩成适合微信阅读的摘要

这些前缀只会增强发给 OMO 的 prompt，不会改变普通消息、同步游标或微信回复机制。`#plan` 的最近一次结果会按用户单独缓存，供后续 `#start` 续跑使用。
