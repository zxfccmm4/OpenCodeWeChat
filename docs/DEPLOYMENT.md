...
- 不设置时 OpenCodeWeChat 不传 `model` 字段，由 OpenCode / OMO 按自己的配置选择模型
- 通常不要同时设置 `OPENCODE_AGENT` 和 `OPENCODE_PROVIDER_ID` / `OPENCODE_MODEL_ID`
- 遇到 `ProviderModelNotFoundError` 时，先删除 `OPENCODE_PROVIDER_ID` / `OPENCODE_MODEL_ID` 后重启
